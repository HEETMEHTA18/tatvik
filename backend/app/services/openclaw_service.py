...
class OpenClawService:
    def __init__(self):
        ...
        if not self.enabled:
            logger.warning("OpenClaw API Key is not configured. OpenClaw execution will run in dry-run mode.")

    async def execute_task(self, repo_url: str, task_description: str, branch_name: str = "main") -> dict:
        """
        ...
        """
        if not self.enabled:
            logger.info(f"[Stub] Executing task '{task_description}' on {repo_url} (branch: {branch_name})")
            return {
                "success": True,
                "stub": True,
        ...
        payload = {
            ...
            "branch": branch_name,
            "github_token": getattr(settings, "github_client_secret", "")  # Passed for GitHub PR authorization
        }
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                logger.exception("Failed to dispatch task execution request to OpenClaw")
        ...
        async with httpx.AsyncClient() as client:
            try:
                ...
                if response.status_code == 200:
                    ...
                    logger.error(f"OpenClaw command execution returned error: {response.text}")
                    return {"success": False, "error": response.text}
            except Exception as e:
